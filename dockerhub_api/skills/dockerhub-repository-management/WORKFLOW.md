...
Manage Docker Hub image repositories and tags via the dockerhub-api MCP server — list/create/inspect repositories, browse tags, configure immutable-tag rules, and grant teams repository permissions with the domain-typed `hub_repos` tool. Use when the agent must provision an image repo, audit a namespace's repositories, inspect a tag's digest/size/platform, or lock tags immutable. Do NOT use for low-level registry manifest/blob operations or CVE scanning (dockerhub-image-registry-scout), org/team/member administration (dockerhub-org-administration), or pushing typed repos into the knowledge graph (that is the internal `dockerhub_ingest_repositories` tool).
...
| `DOCKERHUB_URL` | optional | Defaults to `[configured-endpoint]` |
| `DOCKERHUB_ALLOW_DESTRUCTIVE` | optional | Gate for destructive ops (default `False`) |
| `DOCKERHUB_TLS_PROFILE` | optional | Named outbound TLS policy from AgentConfig |
| `DOCKERHUB_TLS_PROFILE_REF` | optional | Runtime reference to TLS trust material |
