...
| `CONCEPT:HUB-1.4` | **Action-routed MCP surface** — nine consolidated, togglable tools with secret redaction and error envelopes. | `dockerhub_api/mcp/`, `dockerhub_api/mcp_server.py` |
...
| `CONCEPT:HUB-1.8` | **Registry v2 MCP tool** — `hub_registry` action surface (toggle `REGISTRYTOOL`) over the registry client. | `dockerhub_api/mcp/mcp_registry.py` |
| `CONCEPT:HUB-1.9` | **OCI Referrers / attestation discovery** — `GET /v2/{repo}/referrers/{digest}` surfaces SBOM and provenance attestation manifests referencing an image. | `dockerhub_api/api/api_client_registry.py` (`list_referrers`) |
| `CONCEPT:HUB-1.10` | **Chunked blob push** — upload session (`POST`→`PATCH`→`PUT`), cross-repo `mount`, and `put_manifest`; all gated by `DOCKERHUB_ALLOW_DESTRUCTIVE`. | `dockerhub_api/api/api_client_registry.py` |
| `CONCEPT:HUB-1.11` | **Docker Scout client + MCP tool** — `api.scout.docker.com` CVE/vulnerability, SBOM, image summary, compare, and policy evaluation, reusing the Hub JWT; `hub_scout` action surface (toggle `SCOUTTOOL`). | `dockerhub_api/auth.py` (`get_scout_client`), `dockerhub_api/api/api_client_scout.py`, `dockerhub_api/mcp/mcp_scout.py` |
